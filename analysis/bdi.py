"""三項分解 B+D+I と、クラスタ整合な推論、頑健性チェック。

  S_sim − S_prod = (S_indep − S_prod) + (S_conj − S_indep) + (S_sim − S_conj)
                 =        B          +        D          +        I
  B: clean 正規化（S_prod = S0·Π(Sa/S0) が独立基準からどれだけ持ち上がっているか）
  D: 軸間の生存依存性  I: 重畳効果（同時データが入る唯一の項）

推論は 40 個の (suite,task) セルを単位としたクラスタブートストラップに統一する。
"""
...
def terms(d, L, keys):
    """keys: (suite,task,level,config) の集合 → (B, D, I, S_prod, S_indep, S_conj, S_sim, N, b, c)"""
    u = d["unit"]; N = len(keys)
    if N == 0: return None
...
def clusterboot(d, L, keys, idx, n=4000, seed=0):
    """40 個の (suite,task) を単位に再標本化。idx=0..2 が B,D,I。"""
...
print("## 1. 三項分解  S_sim − S_prod = B + D + I")
print(f"{'方策':12s}{'L':3s}{'S_prod':>7s}{'S_ind':>7s}{'S_conj':>7s}{'S_sim':>7s} |"
      f"{'B':>7s}{'D':>7s}{'I':>7s} | {'I の 95%CI':>17s}{'p':>7s} | {'従来残差':>8s}")
print("-"*104)
RES = {}
for name, *_ in POL:
    d = M[name]
...
print("\n## 2. スイート別 I と leave-one-suite-out I（符号が保たれるか）")
print(f"{'方策/L':17s}{'全体':>7s} | " + "".join(f"{SH[s]:>7s}" for s in SU) +
      " | " + "".join(f"{'¬'+SH[s]:>8s}" for s in SU) + "  符号保持")
print("-"*104)
for name, L in KEY:
    ...
    print(f"{name+'/'+L:17s}{full:+7.1f} | " +
          ...
          ("   ✓ 4/4" if sg else "   ✗"))
print("\n## 3. leave-one-task-out（40 タスクのどれを抜いても符号が保たれるか）")
for name, L in KEY:
    ...
    print(f"  {name+'/'+L:17s} I={full:+6.1f}  符号保持 {same}/{len(tks)}  "
          f"範囲 [{min(vals):+.1f}, {max(vals):+.1f}]")

print("\n## 4. MINERVA の language 軸を使った確率的ゆらぎの負対照")
print("  MINERVA は言い換えを受理せずタスク番号を引くので、L1/L2/L3 は**同一入力・同一初期状態**。")
print("  違うのはロールアウトのシードだけ。つまり同一条件の反復実行に相当する。")
...
for a, b_ in (("L1","L2"),("L1","L3"),("L2","L3")):
    ...
    print(f"    {a} vs {b_}: 成否の不一致 {100*dis/len(ks):.1f}%  ({dis}/{len(ks)})   step 差の中央値 {sd:.0f}")
...
print(f"    clean vs L1: 成否の不一致 {100*dis/len(kk):.1f}%  ({dis}/{len(kk)})")
print(f"  → 完全同条件での成否反転はこの水準。主要な |I| は 4〜14 pp。")

print("\n## 5. 学習シードによる I のばらつき（MINERVA abl_l1 3シード）")
seeds = {}
...
print(f"  {'L':4s}" + "".join(f"{'s'+str(s):>9s}" for s in seeds) + f"{'SD':>8s}{'方策間の幅':>10s}")
for L in LV:
...
print("\n## 6. MINERVA の language 軸を除いた 5 軸での分解")
...
for L in LV:
    ...
    print(f"  {L}: 6軸 D={D6:+.1f} I={I6:+.1f}  →  5軸(language 除く) D={D5:+.1f} I={I5:+.1f}")
